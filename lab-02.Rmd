...
author: "Team Name: Leonardo Maffei Mercalli, Aksel Huillard, Ben Calder"
date: "`r Sys.Date()`"
...
regiondata <- plastic_data_all %>% select(region)
table(regiondata)
...
meantotal <- plastic_data_all %>% group_by(region) %>% summarise(mean = mean(total_mismanage_plastic))
...
medtotal <- plastic_data_all %>% group_by(region) %>% summarise(median = median(pct_mismanaged_plastic_ocean))%>% drop_na()
...
plastic_data_all <- plastic_data_all %>% rename("pop" = population)

plastic_data_all <- plastic_data_all %>% mutate(region = str_replace_all(region, "Latin America and The Caribbean", "LAC"))
